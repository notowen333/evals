...
def main() -> int:
    ...
    parser.add_argument("--container-workspace-path", default="/testbed", help="Workdir inside the task container ({workdir}).")
    ...
    runner_argv = [
        "runner.py",
        "--agent", _AGENT_SPEC,
        "--instruction", args.problem_statement,
    ...
    try:
        ...
        print(f"run_infer: failed to write vibe_metrics.json: {exc}", file=sys.stderr)

    # ATIF trajectory.json (alongside the raw conversation.json). Best-effort.
    try:

        write_atif_trajectory(output_dir, result, model_name=args.model_name)
    except Exception as exc:  # noqa: BLE001 — never fail the run over a trajectory artifact
        print(f"run_infer: failed to write trajectory.json: {exc}", file=sys.stderr)
